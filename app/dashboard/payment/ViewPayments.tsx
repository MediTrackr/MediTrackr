...
const METHOD_META: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  cash:          { label: "Comptant",       color: "text-green-400",  icon: <Banknote className="w-3.5 h-3.5" /> },
  check:         { label: "Chèque",         color: "text-yellow-400", icon: <FileCheck className="w-3.5 h-3.5" /> },
  credit_card:   { label: "Carte crédit",   color: "text-blue-400",   icon: <CreditCard className="w-3.5 h-3.5" /> },
  debit_card:    { label: "Carte débit",    color: "text-blue-300",   icon: <CreditCard className="w-3.5 h-3.5" /> },
  bank_transfer: { label: "Virement",       color: "text-purple-400", icon: <Building2 className="w-3.5 h-3.5" /> },
  stripe:        { label: "Stripe",         color: "text-indigo-400", icon: <CreditCard className="w-3.5 h-3.5" /> },
  ramq:          { label: "RAMQ",           color: "text-primary",    icon: <FileCheck className="w-3.5 h-3.5" /> },
};
...
function groupPayments(list: Payment[], by: GroupBy): [string, Payment[]][] {
  const map = new Map<string, Payment[]>();
  list.forEach(p => {
    let key: string;
    if (by === "date")   key = monthKey(p.payment_date);
    else if (by === "method") key = METHOD_META[p.payment_method ?? ""]?.label ?? "Autre";
    else key = p.source || "Source inconnue";
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      list = list.filter(p =>
        ...
        METHOD_META[p.payment_method ?? ""]?.label.toLowerCase().includes(q)
      );
    }
    return list;
  }, [payments, search]);
    ...
    <div className="min-h-screen bg-black flex justify-center items-start p-4 pt-6">
      <div className="w-full max-w-7xl bg-[#050505] rounded-[2.5rem] relative
        ...
        <div className="relative z-10 m-4 sm:m-6 mb-0 p-4 sm:p-5 bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl flex flex-wrap gap-3 justify-between items-center">
          <div>
            <h1 className="text-2xl font-black text-green-400 uppercase italic tracking-tighter leading-none">Paiements</h1>
            <p className="text-xs text-white/30 mt-0.5">{payments.length} paiement{payments.length !== 1 ? "s" : ""} enregistré{payments.length !== 1 ? "s" : ""}</p>
          </div>
        ...
        <div className="relative z-10 mx-4 sm:mx-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Total reçu",     value: `$${stats.total.toFixed(2)}`,       color: "text-green-400",  border: "border-green-500/20" },
            { label: "Ce mois",        value: `$${stats.thisMonth.toFixed(2)}`,   color: "text-primary",    border: "border-primary/20" },
            { label: "À encaisser",    value: `$${stats.outstanding.toFixed(2)}`, color: "text-blue-400",   border: "border-blue-500/20" },
            { label: "Factures ouv.", value: stats.pendingCount,                   color: "text-red-400",    border: "border-red-500/20" },
          ].map(s => (
        ...
        {methodTotals.length > 0 && (
          <div className="relative z-10 mx-4 sm:mx-6 flex gap-2 overflow-x-auto pb-1">
            {methodTotals.map(([method, amt]) => {
              const meta = METHOD_META[method] ?? { label: method, color: "text-white/40", icon: <DollarSign className="w-3.5 h-3.5" /> };
              return (
                <div key={method} className="flex items-center gap-2 bg-black/40 border border-white/8 rounded-xl px-3 py-2 whitespace-nowrap shrink-0">
                  ...
                  <div>
                    <p className="text-[9px] uppercase font-bold text-white/25">{meta.label}</p>
        ...
        <div className="relative z-10 mx-4 sm:mx-6 flex gap-1 bg-black/40 border border-white/10 rounded-2xl p-1">
          {([
            ["all",      "Tous les paiements", <CheckCircle2 key="a" className="w-3.5 h-3.5" />, "text-green-400"],
            ["pending",  "Factures ouvertes",  <Clock key="p" className="w-3.5 h-3.5" />,        "text-blue-400"],
        ...
        <div className="relative z-10 mx-4 sm:mx-6">
          <div className="relative">
            ...
            <input
              ...
              placeholder={tab === "pending" ? "Rechercher patient, numéro de facture..." : "Rechercher source, référence..."}
          ...
          <div className="relative z-10 mx-4 sm:mx-6 flex flex-col gap-2">
            {filteredPending.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-white/30 text-sm">{search.trim() ? "Aucun résultat" : "Aucune facture en attente"}</p>
              </div>
            ) : (
              filteredPending.map(inv => {
                ...
                return (
                  <div key={inv.id} className={`bg-black/40 border rounded-2xl p-4 sm:p-5 flex flex-wrap items-center gap-4 ${isOverdue ? "border-red-500/30" : "border-blue-500/20"}`}>
                    ...
                    <div>
                      <p className="text-[9px] uppercase font-bold text-white/25 mb-0.5">À payer</p>
                      <p className={`text-base font-black ${isOverdue ? "text-red-400" : "text-blue-400"}`}>${outstanding.toFixed(2)}</p>
                    </div>
                    {inv.due_date && (
                      <div>
                        <p className="text-[9px] uppercase font-bold text-white/25 mb-0.5">Échéance</p>
            ...
            <div className="relative z-10 mx-4 sm:mx-6 flex justify-end">
              <div className="flex gap-1 bg-black/40 border border-white/10 rounded-xl p-1">
                {([["date",   <Calendar key="c" className="w-3.5 h-3.5" />, "Date"],
                   ["method", <Tag key="t" className="w-3.5 h-3.5" />, "Mode"],
                   ["source", <MapPin key="m" className="w-3.5 h-3.5" />, "Source"]] as [GroupBy, React.ReactNode, string][]).map(([key, icon, lbl]) => (
                  <button
                    key={key}
            ...
            <div className="relative z-10 mx-4 sm:mx-6 flex flex-col gap-4">
              {loading ? (
                ...
                <div className="text-center py-16">
                  ...
                  <Link href="/dashboard/payment/new">
                    <Button className="bg-green-500 text-black mt-4 text-xs">Enregistrer un paiement</Button>
                  </Link>
                </div>
              ) : (
                groups.map(([groupLabel, items]) => (
                  <div key={groupLabel} className="flex flex-col gap-2">
                    ...
                    {items.map(p => {
                      const meta = METHOD_META[p.payment_method ?? ""] ?? { label: p.payment_method || "—", color: "text-white/40", icon: <DollarSign className="w-3.5 h-3.5" /> };
                      return (
                        <div
                          key={p.id}
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-4">
                            ...
                            <div className={`flex items-center gap-2 min-w-[100px] ${meta.color}`}>
                              {meta.icon}
                              <div>
                                <p className="text-[9px] uppercase font-bold text-white/25 mb-0.5">Mode</p>
                            ...
                            {p.source && (
                              <div className="flex-1 min-w-[100px]">
                                <p className="text-[9px] uppercase font-bold text-white/25 mb-0.5">Source</p>
                            ...
                            {p.reference_number && (
                              <div>
                                <p className="text-[9px] uppercase font-bold text-white/25 mb-0.5">Réf.</p>
