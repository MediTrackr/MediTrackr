...
export default function RemittancePage() {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const res = await fetch("/api/ocr/remittance", {
        method: "POST",
  ...
  // Apply parsed results: update matched claims with amount_received
  const applyResults = async () => {
    ...
    try {
      for (const line of parseResult.lines) {
        ...
        const newStatus = line.amount_withheld && line.amount_withheld > 0 ? "partial" : "paid";
        await supabase
          .from("ramq_claims")
          .update({
        ...
        // Mark remittance line as applied
        await supabase
      ...
      <div className="w-full max-w-6xl min-h-[95vh] bg-[#050505] rounded-[2.5rem] relative
        ...
        <div className="sticky top-0 z-20 flex items-center justify-between px-8 py-5
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1.5 text-[9px] px-2.5 py-1.5 rounded-full
                             bg-primary/10 border border-primary/20 text-primary font-semibold uppercase tracking-wider">
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          {!parseResult && !applyDone && (
            <div
              className={`relative rounded-3xl border-2 border-dashed transition-all cursor-pointer p-12
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <button
                  ...
                  disabled={applying || parseResult.matchedCount === 0}
...
function ConfidenceBadge({ value }: { value: number }) {
  ...
  return (
    <span className={`text-[9px] font-bold px-2.5 py-1 rounded-full border ${color}`}>
      {value}% confiance
    </span>
  );
}
    ...
    <div className={`rounded-2xl border transition-all ${
      ...
      >
        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${
          line.matched ? hasWithheld ? "bg-yellow-400" : "bg-green-400" : "bg-white/20"
        }`} />
        <div className="flex-1 min-w-0 grid grid-cols-[1fr_auto_auto_auto] items-center gap-4">
          <div className="min-w-0">
            <p className="text-sm text-white/80 font-medium truncate">
              {line.patient_ramq ?? line.claim_number ?? "—"}
            </p>
            <p className="text-[10px] text-white/30 mt-0.5">
              {line.service_date ?? "—"}
        ...
        {expanded ? <ChevronUp className="w-3.5 h-3.5 text-white/20 flex-shrink-0" />
...
function DetailField({ label, value, valueClass = "text-white/60" }: { label: string; value: string; valueClass?: string }) {
  return (
    <div>
      <p className="text-[9px] text-white/25 uppercase tracking-wider mb-0.5">{label}</p>
      <p className={`font-mono text-[11px] ${valueClass}`}>{value}</p>
    </div>
  );
}
